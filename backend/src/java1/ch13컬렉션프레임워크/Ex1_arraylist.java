...
public class Ex1_arraylist {
	public static void main(String[] args) {
		...
		//1. 컬렉션 프레임 워크 객체 [클래스명 <타입파라미터>변수명 = new 클래스명 <>();]
			//<> : 클래스 안에 클래스를 매개로 넣는 방식 
		...
		// 내가 만든 자료형(member)을 여러개를 저장할 수 있는 리스트 선언
		...
		for(String s : list) {	//for (타입파라미터)
			//list에서 string을 꺼내오겠다
			System.out.println(s);
		
		
		//7.list.contains : 해당 객체가 존재하면 true/false
		System.out.println(list.contains("jdbc"));
		
		//8.indexOf : 해당 객체의 인덱스 위치 반환
		System.out.println(list.indexOf("sad"));	
		
		//9. list.isempty() : 리스트가 객체가 하나도 없는지 확인  하나도 없으면 true/false
